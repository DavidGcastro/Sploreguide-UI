    	...
    	<Animated.View style={[styles.headerBackground, this.animateHeader, { height: max-150 }]}>;
		        ...
		        <View style={{flex: 1, flexDirection: 'column', zIndex:2, position:'absolute'}}>
			        ...
			        <View style={{flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: 'transparent', width:width, marginTop: isIphoneX() ? 55 : 30}}> {/* marginTop should be 55 for iPhone X */}
			        	<Animated.View style={[{justifyContent: 'flex-end', marginLeft: 24}, this.animateTopBar]}>
			        			<NavBackButton title='' onPress={this.props.onBackPress} />
			        	</Animated.View>

			        	<Animated.View style={[{justifyContent: 'flex-end', marginLeft: 24}, this.animateTopBar]}>
			        		<Tag activity_type={activity_type} color={color} fontSize={12}/>
			        	</Animated.View>

			        	<View style={{flex: 6, alignItems: 'flex-end', marginRight: 24}}>
	                ...
	                <Ionicons name={'ios-share-outline'} size={26} color={'white'} />
			        	...
			        	<View style={{marginRight: 28}}>
			        		<Ionicons name={'ios-heart-outline'} size={26} color={'white'} />
			        	</View>
			        </View>

			        {/* Subtitle */}
			        <View style={{flex: 1, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-end', width:width}}> 
			        	<View style={{marginLeft: 22, marginTop: 20}}>
	                <Text style={{fontFamily: 'SF-UI-Text-Regular', fontSize: 18, color: 'white'}}>{this.parseLocation(location)}</Text>
	              </View>
	            </View>
	             ...
	             <View style={{flex: 1, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-end', width:width}}> 
	                ...
	                <Text style={{fontFamily: 'SF-UI-Text-Semibold', fontSize: 26, lineHeight: 26, color: 'white'}}>{title}</Text>
	              </View>
	            </View>

	            {/* Main Details */}
	        		<View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', width:width}}> 
			        	<View style={{flex: 2, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-end' }}>
									<View style={{marginLeft: 22}}>
										<Duration style={{fontFamily: 'SF-UI-Text-Regular', fontSize: 12, color: 'rgba(255, 255, 255, 0.5)', paddingTop: 10}} duration={duration}/>
									</View>
									<View style={{marginLeft: 22}}>
										<Language style={{fontFamily: 'SF-UI-Text-Regular', fontSize: 12, color: 'rgba(255, 255, 255, 0.5)', paddingTop: 10}} language={language}/>
	             		</View>
	              </View>
	              <View style={{marginRight: 22, flex: 1}}>
									<Reviews style={{fontFamily: 'SF-UI-Text-Regular', fontSize: 12, color: 'rgba(255, 255, 255, 0.5)', paddingTop: 10}} data={reviews}/>
	              </View>
	            </View>

	            {/* Pagination Indicator */}
	            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width:width, top:18}}> 
	          			{barArray}
	        		</View>

			        <View style={{flex: 0.75, backgroundColor: 'green', width:width}}></View>
